...
const Overview = ({
  ...
  <>
    {memberSupport && (
      <PercentageBar title="Member Support" percentage={memberSupport} />
    )}
    {tokenWeightedSupport && (
      <PercentageBar
        title="Token Weighted Support"
        percentage={tokenWeightedSupport}
      />
    )}
  </>
);
...
export const ProposalStats = () => {
  ...
  const navItems = Object.keys(views).map((title) => ({
    title,
