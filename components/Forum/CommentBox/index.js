...
export default function CommentBox({ proposal, provider }) {
    ...
    const submitToSnapshot = () => vote(provider)({ choice: selectedChoice, proposalId: proposal.id, voteType: proposal.type })
    const submitAndVote = () => { submitToSnapshot(); postComment(); }
    ...
    return (
      <div className="flex flex-col space-y-6 p-6 bg-white rounded-lg border border-gray-200 shadow-lg dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="flex flex-row space-x-3">
          ...
          <Button title="Post + Vote" color="purple" icon={true} onClick={submitAndVote} />
        </div>
      </div>
    )
