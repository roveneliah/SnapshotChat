...
export default function JerrySearch(props) {
  ...
  return (
    <div className="flex h-[85vh] flex-col space-y-4 overflow-auto rounded-lg border border-gray-200 bg-cards bg-opacity-75 p-8 shadow-xl dark:border-gray-700 dark:bg-gray-800">
        ...
        <div className="grid h-full grid-cols-1 overflow-auto">
          {searchResults
            ...
            .map((profile, i) => (
              <div
                key={i}
                className="my-2 flex h-fit flex-col justify-between space-y-5 rounded-lg bg-cards bg-opacity-25 p-6 shadow-md dark:border-gray-700 dark:bg-gray-800"
              >
                ...
                /> */}
                  <span
                    onClick={() => props.userProfile.follow(profile.address)}
                    className="mr-2 cursor-pointer rounded bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-800 dark:bg-gray-200 dark:text-gray-900"
                  >
                    Follow
                  </span>
                </div>
