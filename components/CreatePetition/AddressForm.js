...
export const AddressForm = ({ addSigner }) => {
	...
	const addIfValidSigner = () => getIsValidAddress(addressInput)
		.then(() => {
			addSigner({ address: addressInput, signer: name })
		})
		.catch(() => setIsInvalidAddress(true))
	...
	const neutralForm = {
		label: "block mb-2 text-sm font-medium text-gray-700 dark:text-gray-500 placeholder:text-gray-400",
		input: "bg-gray-50 border border-gray-300 text-gray-900 placeholder-gray-400 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-400",
	}
		...
		<div>
			<div className="flex flex-row space-x-3 justify-evenly">
				<div className="mb-6 w-1/2">
					<label className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-500">Name</label>
					<input 
            autoComplete="off"
						value={name} 
						onChange={compose(setName, prop("value"), prop("target"))} type="text" id="username-success" 
